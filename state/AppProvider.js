...
export function AppProvider({ children }) {
    ...
    await asyncForEach(args, async (arg) => {
      ...
      try {
        jwt = await litNodeClient.getSignedToken({
          ...
          authSig,
          accessControlConditions:
            arg.accessControlConditions.accessControlConditions,
  ...
  const joinSpace = async (space, isInGame = false) => {
    ...
    const store = await storeUserPermittedResources({
      authSig,
      jwts: validJwts,
      spaceId,
    });
    ...
    const queryGatherUrl = {
      gatherUrl: window.location.origin + window.location.pathname,
    };
    ...
    const redirectUrl = getGatherRedirectUrl({
      ...
      queries: [queryAuthSig, queryGatherUrl, querySpace, queryIsInGame],
    });
  ...
  return (
    <AppContext.Provider value={sharedState}>
      {/* --- Global Loading --- */}
      <div
        className={`fixed top-0 w-full bg-color-gather-gradient py-1 flex justify-center transition pointer-events-none`}
        style={{ zIndex: 99, opacity: loaded ? 0 : 1 }}
      >
      ...
      {!hasWeb3Wallet ? (
        <div className="w-full text-center text-red">
          No web3 wallet was found. Please connect to your wallet and refresh
          the page
        </div>
      ) : (
        ""
